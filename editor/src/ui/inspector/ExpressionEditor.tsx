...
export function ExpressionEditor({ expression, dispatch }: { expression: ExpressionAssignment; dispatch: (action: EditorAction) => void }) {
  ...
  return (
    <section className="expression-editor" aria-labelledby="expression-editor-title">
      <div className="inspector-subheading"><h3 id="expression-editor-title">Expression pedal</h3><label className="inline-check"><input type="checkbox" checked={expression.enabled} onChange={event => patch({ enabled: event.target.checked })} /> Enabled</label></div>
      {expression.enabled ? <div className="expression-compact-fields">
      ...
      </div> : <p className="section-summary">Disabled for this bank.</p>}
    </section>
  );
}
